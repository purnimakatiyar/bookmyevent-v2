...
class TestEventHelper:
    ...
    def test_add_event(self, username, event_details):
        ...
        with patch("utils.input.Input.add_event_input", mock_add_event_input):
            ...
            with patch("controllers.user.User.get_user_id", mock_user_id):
                helper = EventHelper()
                result = helper.add_event(username)
    ...
    def test_remove_event_with_existing_event(self, username, event_name):
        ...
        with patch("utils.input.Input.remove_event_input", mock_remove_event_input):
            ...
            with patch("controllers.user.User.get_user_id", mock_user_id):
                ...
                with patch("controllers.event.Event.get_event_by_user", mock_get_event_by_user):
                    mock_remove_event = MagicMock()
                    with patch("controllers.event.Event.remove_event", mock_remove_event):
                        event_helper = EventHelper()
                        result = event_helper.remove_event(username)
    ...
    def test_list_events_with_existing_events(self, username, mock_user_id):
        with patch("controllers.user.User.get_user_id", mock_user_id):
            ...
            with patch("controllers.event.Event.list_events", mock_list_events):
                event_helper = EventHelper()
    ...
    def test_list_all_events(self):
        ...
        with patch("controllers.event.Event.list_all_events", mock_list_all_events):
            event_helper = EventHelper()
            result = event_helper.list_all_events()
        ...
        with patch('controllers.user.User.get_user_id', mock_user_id):
            with patch('utils.input.Input.update_event_input', return_value=update_event_details):
                ...
                with patch('controllers.event.Event.update_event', mock_update_event):
                    event_helper = EventHelper()
                    result = event_helper.update_event(username, choice)
    ...
    def test_view_booked_event_not_exists(self, username, mock_user_id):
        with patch("controllers.user.User.get_user_id", mock_user_id):
            ...
            with patch("controllers.booked_events.BookedEvents.view_booked_events", mock_view_booked_events):
                event_helper = EventHelper()
    ...
    def test_update_ticket(self):
        ...
        with patch('controllers.event.Event', mock_event_class):
            event_helper = EventHelper()
            result = event_helper.update_ticket(5, "EventName")
