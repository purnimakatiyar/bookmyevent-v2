...
class TestUserHelper:
    ...
    def test_signup_user_exists(self):
        ...
        with patch('helpers.input.Input.signup_input', mock_signup):
            with patch('controllers.user.User.signup', return_value="user"):
    ...
    def test_signup_user_not_exists(self):
        mock_signup = MagicMock(return_value="signup_details")
        with patch('helpers.input.Input.signup_input', mock_signup):
            with patch('controllers.user.User.signup', return_value=None):
                user_helper = UserHelper()
                user_helper.signup('role')
    ...
    def test_remove_manager_not_exists(self):
        ...
        with patch('helpers.input.Input.remove_manager_input', mock_remove_manager):
            with patch('controllers.user.User.get_user', return_value=None):
                user_helper = UserHelper()
    ...
    def test_remove_manager_exists(self):
        mock_remove_manager = MagicMock(return_value='username')
        with patch('helpers.input.Input.remove_manager_input', mock_remove_manager):
            with patch('controllers.user.User.get_user', return_value='manager'):
                user_helper = UserHelper()
                user_helper.remove_manager()
        assert mock_remove_manager.called is True
    ...
    def test_update_account(self, username, choice, update_user_details, mock_user_id):
        with patch('controllers.user.User.get_user_id', mock_user_id):
            with patch('helpers.input.Input.update_account_input', return_value=update_user_details):
                mock_update_user = MagicMock(return_value=True)
                with patch('controllers.event.Event.update_event', mock_update_user):
